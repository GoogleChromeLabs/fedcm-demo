...
This project is a demonstration of a **Relying Party (RP)** implementing the [Federated Credential Management (FedCM) API](https://developer.chrome.com/docs/identity/fedcm/overview).

It is designed to work alongside a FedCM-compatible [Identity Provider (IdP)](https://github.com/GoogleChromeLabs/fedcm-demo/tree/main/fedcm-idp-demo) to showcase how a user can sign in to a website (the RP) using their existing account from another service (the IdP). This process happens through a browser-mediated flow with the FedCM API.
...
This demo is part of a pair. You will also need to run the corresponding **[IdP Demo](../fedcm-demo-idp)** for the full sign-in flow to work.
...
```
127.0.0.1       fedcm-demo-idp.localhost
127.0.0.1       fedcm-demo-rp.localhost
```
...
This script will start Caddy, which automatically handles HTTPS certificate generation and trust for `fedcm-demo-idp.localhost` and `fedcm-demo-rp.localhost`. The first time you run it, Caddy will ask for your system password to install a local certificate authority (CA) into your browser's trust store.
...
Create a new file in the current fedcm/fedcm-demo-rp file named .env.
...
```
PROVIDER_URLS=["https://fedcm-demo-idp.localhost"]
RP_URL=https://fedcm-demo-rp.localhost
IDP1_URL=https://fedcm-demo-idp.localhost
```

### 4. Run the demos
Now make sure that the demos run on the right ports specified in the `/caddy/Caddyfile`. You can also modify thoe as needed, but the default is:
* RP: 8080
* IdP: 8081
...
git init fedcm-demo-rp-project
cd fedcm-demo-rp-project

# Add the remote repository
git remote add origin https://github.com/GoogleChromeLabs/fedcm-demo.git

# Configure sparse checkout to only pull the RP demo directory
git sparse-checkout set fedcm/fedcm-demo-rp

# Pull the files from the main branch
git pull origin main

# Navigate into the demo directory to run commands
cd fedcm/fedcm-demo-rp
```
